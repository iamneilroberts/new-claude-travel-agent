...
oauthApp.get("/oauth/authorize", async (c) => {
  ...
  try {
    ...
    console.error('Database error in OAuth authorize:', error);
  ...
  try {
    ...
    if (action === 'approve') {
      ...
      try {
        ...
        const oauth2Model = new OAuth2Model(c.env);
        ...
        console.error('Database error in OAuth consent:', dbError);
...
oauthApp.post("/oauth/authorize", async (c) => {
  try {
    const oauth2Server = createOAuth2Server(c.env);
        ...
        authenticateHandler: {
          handle: async (request: OAuth2Server.Request) => {
            ...
            try {
              ...
              console.error('Database error in OAuth authorize user lookup:', dbError);
              return null;
            }
          }
        }
...
oauthApp.post("/oauth/applications", async (c) => {
  try {
    ...
    for (const uri of redirect_uris) {
      // Allow standard OAuth 2.0 out-of-band redirect URI
      ...
      try {
        ...
        if (url.protocol !== 'https:' && !url.hostname.includes('localhost') && !url.hostname.includes('127.0.0.1')) {
          return c.json({
            ...
            error_description: "Only HTTPS redirect URIs are allowed (except localhost and urn:ietf:wg:oauth:2.0:oob)"
          }, 400);
        }
      } catch {
        return c.json({
