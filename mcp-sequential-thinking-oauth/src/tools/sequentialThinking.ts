...
export function registerSequentialThinkingTool(server: McpServer, userContext: UserContext, env: Env) {
  server.tool(
    "sequential_thinking", 
    "Perform step-by-step systematic analysis of problems", 
    { 
    ...
    async ({ problem, context = '', steps = 5, format = 'markdown' }) => {
      try {
        ...
        for (let i = 1; i <= steps; i++) {
          thinking.steps.push({
            ...
            content: generateStepContent(i, problem, context)
          });
        }
        ...
        if (format === 'structured') {
          ...
          result = formatAsMarkdown(thinking);
        } else {
          result = formatAsText(thinking);
        }
...
function generateStepContent(stepNumber: number, problem: string, context: string): string {
  const stepContent = [
...
function formatAsMarkdown(thinking: any): string {
...
function formatAsText(thinking: any): string {
