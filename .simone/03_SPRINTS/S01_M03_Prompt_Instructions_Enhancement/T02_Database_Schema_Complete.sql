-- T02_S01: Complete Database Schema for Chain Execution and Template Variables
-- Date: January 8, 2025
...
-- =============================================================================
-- This schema extends the existing prompt-instructions-mcp server with:
...
CREATE TABLE template_processings (
  id TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(16)))),
...
-- =============================================================================
-- ENHANCE EXISTING TABLE (BACKWARD COMPATIBLE)
-- =============================================================================
...
CREATE INDEX idx_chains_name_active ON execution_chains(name) WHERE is_active = true;
CREATE INDEX idx_chains_updated ON execution_chains(updated_at DESC);

-- template_definitions indexes  
CREATE INDEX idx_templates_category_active ON template_definitions(category, is_active);
CREATE INDEX idx_templates_name_active ON template_definitions(name) WHERE is_active = true;
CREATE INDEX idx_templates_updated ON template_definitions(updated_at DESC);

-- chain_executions indexes
CREATE INDEX idx_executions_status_started ON chain_executions(status, started_at DESC);
CREATE INDEX idx_executions_chain_started ON chain_executions(chain_id, started_at DESC);
CREATE INDEX idx_executions_session ON chain_executions(session_id, started_at DESC);
CREATE INDEX idx_executions_completed ON chain_executions(completed_at DESC) WHERE status IN ('completed', 'failed');

-- template_processings indexes
CREATE INDEX idx_processings_template_created ON template_processings(template_id, created_at DESC);
CREATE INDEX idx_processings_session ON template_processings(session_id, created_at DESC);
CREATE INDEX idx_processings_performance ON template_processings(processing_time_ms);

-- Enhanced instruction_sets indexes
CREATE INDEX idx_instructions_templating ON instruction_sets(supports_templating, is_active) WHERE supports_templating = true;
CREATE INDEX idx_instructions_chain_compatible ON instruction_sets(chain_compatible, is_active) WHERE chain_compatible = true;
...
INSERT INTO template_definitions (name, title, content, variables_schema, category, default_values, usage_examples) VALUES
(
...
-- =============================================================================
-- SEED DATA: STARTER EXECUTION CHAINS
-- =============================================================================

-- Travel workflow chains
INSERT INTO execution_chains (name, title, description, category, steps, variables_schema, default_variables) VALUES
(
...
-- These queries should return expected results after migration
...
-- This will remove all new features and restore original state

-- Drop new columns from instruction_sets
...
-- =============================================================================
-- SCHEMA DESIGN COMPLETION VERIFICATION
-- =============================================================================
...
-- Ready for implementation in S02 sprint!