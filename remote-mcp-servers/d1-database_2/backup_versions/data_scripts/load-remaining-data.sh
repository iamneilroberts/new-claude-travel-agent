...
echo "🛫 Loading remaining airport data to remote D1 database..."

# Continue loading airport chunks from where we left off
for i in {11..86}; do
    echo "Loading airports chunk $i/86..."
    wrangler d1 execute travel_assistant --remote --file=airports_data_chunk_$i.sql
done

echo "🏙️ Loading all city data to remote D1 database..."
...
echo "🎉 D1 remote database setup complete!"