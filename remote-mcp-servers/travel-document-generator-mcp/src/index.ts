/**
 * Travel Document Generator MCP - Database Template System
 *
 * Generates travel documents from HTML templates stored in D1 database
 * with Handlebars rendering and trip data integration.
 */
...
export class TravelDocumentGeneratorMCP extends McpAgent {
  server = new McpServer({
    name: "Travel Document Generator MCP",
    version: "2.0.0",
    description: "Generates travel documents from database templates with Handlebars rendering and trip data integration",
  });

  async init() {
    console.log('TravelDocumentGeneratorMCP: Initializing database template system...');

    // Tool 1: Generate travel document from template
    this.server.tool(
      "generate_travel_document",
      {
        template_id: z.number().int().positive().describe('ID from document_templates table'),
        trip_id: z.number().int().positive().describe('ID from trips table'),
        output_format: z.enum(['html', 'mobile-html']).optional().default('html').describe('Output format'),
        save_to_github: z.boolean().optional().default(false).describe('Save to GitHub repo for web serving')
      },
      async (params) => {
        const env = (this as any).env as Env;
        console.log('TravelDocumentGeneratorMCP: generate_travel_document called with:', params);

        try {
          // 1. Get template from database
          const templateQuery = await env.DB.prepare(
          ...
          // 2. Get trip data
          const tripData = await getComprehensiveTripData(params.trip_id, env.DB);
          ...
          if (params.save_to_github && env.GITHUB_TOKEN) {
            // GitHub integration would go here
            githubResult = {
              html_url: `${env.BASE_URL || ''}/trip-${params.trip_id}/document-${params.template_id}.${params.output_format}`
            };
          }
          ...
          console.error('TravelDocumentGeneratorMCP: Exception generating document:', error);
          return {
            content: [{
              type: "text",
              text: JSON.stringify({
    ...
    // Tool 2: Manage document templates (CRUD operations)
    this.server.tool(
      "manage_document_template",
      {
        action: z.enum(['create', 'update', 'delete', 'list', 'get']).describe('Action to perform'),
        template_id: z.number().int().positive().optional().describe('Required for update/delete/get'),
        ...
        }).optional().describe('For create/update operations'),
      },
      async (params) => {
        const env = (this as any).env as Env;
        console.log('TravelDocumentGeneratorMCP: manage_document_template called with:', params);
        ...
        try {
          ...
          console.error('TravelDocumentGeneratorMCP: Exception managing template:', error);
          return {
            content: [{
              type: "text",
              text: JSON.stringify({
    ...
    // Tool 3: Preview template with sample data
    this.server.tool(
      "preview_template",
      {
        template_id: z.number().int().positive().describe('ID of template to preview'),
        trip_id: z.number().int().positive().optional().describe('Optional trip ID for real data preview'),
        use_sample_data: z.boolean().optional().default(true).describe('Use sample data if trip_id not provided'),
      },
      async (params) => {
        const env = (this as any).env as Env;
        console.log('TravelDocumentGeneratorMCP: preview_template called with:', params);
        ...
        try {
          ...
          let previewData;
          if (params.trip_id) {
            previewData = await getComprehensiveTripData(params.trip_id, env.DB);
            if (!previewData) {
              return {
                content: [{
                  type: "text",
          ...
          } else if (params.use_sample_data) {
          ...
          console.error('TravelDocumentGeneratorMCP: Exception previewing template:', error);
          return {
            content: [{
              type: "text",
              text: JSON.stringify({
    ...
    // Tool 4: Create sample templates for testing
    this.server.tool(
      "create_sample_templates",
      {},
      async () => {
        const env = (this as any).env as Env;
        console.log('TravelDocumentGeneratorMCP: create_sample_templates called');
...
<head>
  ...
  <style>
    ...
    .gallery { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin: 25px 0; }
    ...
    .note { background: #e8f4fd; padding: 15px; border-left: 4px solid #3498db; margin: 15px 0; }
  </style>
</head>
<body>
  <div class="container">
    ...
    <div class="intro-section">
      ...
      <div class="note">
        <strong>Why Somo Travel?</strong> We specialize in creating worry-free travel experiences with personal attention and 24/7 support throughout your journey.
      </div>
    </div>
    ...
    <div class="pricing-options">
      ...
      <div class="option">
        ...
        <ul class="features">
          ...
          <li>Group tours and activities</li>
        </ul>
      </div>
    ...
    {{#if images}}
    <h2>A Preview of Your Journey</h2>
    <div class="gallery">
      {{#each images}}
...
<head>
  ...
  <style>
    ...
    .activity-image { width: 100%; max-width: 300px; height: 150px; object-fit: cover; border-radius: 4px; margin: 10px 0; }
    ...
    .important-info { background: #ffeaa7; border: 1px solid #ffd32a; padding: 15px; border-radius: 4px; margin: 20px 0; }
...
<body>
  <div class="container">
    ...
    <div class="important-info">
      ...
      <ul>
        <li>Keep this itinerary with you during travel</li>
        <li>Check in for flights 24-48 hours in advance</li>
        <li>Verify hotel check-in times and policies</li>
    ...
    <div class="day-section">
      ...
      <div class="activity">
        ...
        {{#if image}}<img src="{{image}}" alt="{{title}}" class="activity-image">{{/if}}
        {{#if booking_reference}}<div class="note">Booking Reference: {{booking_reference}}</div>{{/if}}
      </div>
      {{/each}}
      ...
      <div style="margin-top: 20px;">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 10px; margin-top: 10px;">
          {{#each images}}
          <img src="{{url}}" alt="{{alt}}" style="width: 100%; height: 100px; object-fit: cover; border-radius: 4px;">
          {{/each}}
        </div>
      </div>
      {{/if}}
    ...
    <div class="contact-info">
      ...
      <div class="note" style="color: #bbb;">
        Have an amazing trip! Contact me if you need anything during your travels.
      </div>
    </div>
  </div>
</body>
          ...
          console.error('TravelDocumentGeneratorMCP: Exception creating sample templates:', error);
          return {
            content: [{
              type: "text",
              text: JSON.stringify({
    ...
    // Tool 5: Create photo gallery and manage trip images
    this.server.tool(
      "create_trip_photo_gallery",
      {
        trip_id: z.number().int().positive().describe('Trip ID to create gallery for'),
        places: z.array(z.object({
          place_id: z.string().describe('Google Places ID'),
          name: z.string().describe('Place name'),
          category: z.enum(['destination', 'hotel', 'restaurant', 'activity', 'attraction']).describe('Image category'),
          day_number: z.number().optional().describe('Associated day number if applicable')
        })).describe('Places to fetch photos for'),
        auto_select_primary: z.boolean().optional().default(true).describe('Automatically select a primary destination image')
      },
      async (params) => {
        const env = (this as any).env as Env;
        console.log('TravelDocumentGeneratorMCP: create_trip_photo_gallery called with:', params);
        
        try {
          // Call R2 Storage MCP to create photo galleries for each place
          const galleryPromises = params.places.map(async (place) => {
            try {
              ...
              const galleryResponse = await fetch(`${env.R2_URL_BASE}/create-place-gallery`, {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                  'Authorization': `Bearer ${env.MCP_AUTH_KEY}`
                },
                body: JSON.stringify({
                  trip_id: params.trip_id,
          ...
          const galleryUrl = `${env.R2_URL_BASE}/trip-${params.trip_id}/gallery`;
          ...
          console.error('TravelDocumentGeneratorMCP: Exception creating photo gallery:', error);
          return {
            content: [{
              type: "text",
              text: JSON.stringify({
    ...
    // Tool 6: Save selected images to trip database
    this.server.tool(
      "save_selected_images",
      {
        trip_id: z.number().int().positive().describe('Trip ID'),
        selected_images: z.array(z.object({
          url: z.string().describe('Image URL'),
          caption: z.string().optional().describe('Image caption'),
          category: z.enum(['destination', 'hotel', 'restaurant', 'activity', 'attraction', 'hero']).describe('Image category'),
          is_primary: z.boolean().optional().default(false).describe('Is this the primary trip image'),
          day_number: z.number().optional().describe('Associated day number'),
          place_id: z.string().optional().describe('Google Places ID'),
          alt_text: z.string().optional().describe('Alt text for accessibility')
        })).describe('Array of selected images with metadata')
      },
      async (params) => {
        const env = (this as any).env as Env;
        console.log('TravelDocumentGeneratorMCP: save_selected_images called with:', params);
        ...
        try {
          ...
          await env.DB.prepare('DELETE FROM trip_images WHERE trip_id = ?').bind(params.trip_id).run();
          
          // Insert new selected images
          const insertPromises = params.selected_images.map(async (image) => {
          ...
          return {
            content: [{
              ...
              text: JSON.stringify({
                ...
                primary_images: params.selected_images.filter(img => img.is_primary).length,
                categories: [...new Set(params.selected_images.map(img => img.category))]
              })
            }]
          };
        } catch (error) {
          console.error('TravelDocumentGeneratorMCP: Exception saving images:', error);
          return {
            content: [{
              type: "text",
              text: JSON.stringify({
    ...
    console.log("TravelDocumentGeneratorMCP: Database template system initialized with 6 tools:");
    console.log("  - generate_travel_document: Generate documents from database templates");
    console.log("  - manage_document_template: CRUD operations for templates");
    console.log("  - preview_template: Preview templates with real or sample data");
    console.log("  - create_sample_templates: Create initial HTML templates for testing");
    console.log("  - create_trip_photo_gallery: Fetch Google Places photos and create galleries");
    console.log("  - save_selected_images: Save user-selected images to trip database");
  }
}
  ...
  fetch(request: Request, env: Env, ctx: ExecutionContext) {
    ...
    if (url.pathname === '/health') {
      return new Response(JSON.stringify({
        status: 'ok',
        service: 'Travel Document Generator MCP',
        version: '2.0.0',
        features: [
    ...
    if (url.pathname === "/sse" || url.pathname === "/sse/message") {
      return TravelDocumentGeneratorMCP.serveSSE("/sse").fetch(request, env, ctx);
    }

    // MCP endpoint (fallback)
    if (url.pathname === "/mcp") {
      return TravelDocumentGeneratorMCP.serve("/mcp").fetch(request, env, ctx);
    }
